...
class TaskService:
    ...
    def get_all():
        try:
            tasks = current_app.session.query(Task).all()
    ...
    def get_by_id(task_id):
        try:
            task = current_app.session.query(Task).filter(Task.id == task_id).first()
            if not task:
    ...
    def delete(task_id):
        try:
            ...
            current_app.session.delete(task)
            current_app.session.commit()
            return True, None
        except Exception as e:
            current_app.session.rollback()
